...
uint8_t InitMPU6050(void)
{
	...
	I2C_delay(50000); //约2.5ms延时
	Single_WriteI2C(SMPLRT_DIV, 0x07);	//陀螺仪采样率，典型值：0x00，不分频（8KHZ）
	I2C_delay(50000);            
...
{
	acc.x  = ((((int16_t)MPU6050_Buffer[0]) << 8) | MPU6050_Buffer[1])-57;	//减去零偏
	acc.y  = ((((int16_t)MPU6050_Buffer[2]) << 8) | MPU6050_Buffer[3])-26;
	acc.z  = ((((int16_t)MPU6050_Buffer[4]) << 8) | MPU6050_Buffer[5])-35;// - offset_acc.z;

	gyro.x = ((((int16_t)MPU6050_Buffer[8])  << 8) | MPU6050_Buffer[9])+21;//  - offset_gyro.x;
	gyro.y = ((((int16_t)MPU6050_Buffer[10]) << 8) | MPU6050_Buffer[11])-27;// - offset_gyro.y;
	gyro.z = ((((int16_t)MPU6050_Buffer[12]) << 8) | MPU6050_Buffer[13])+16;// - offset_gyro.z;
